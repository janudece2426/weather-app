...
export default function App() {
  ...
  const fetchWeather = async (city: string, coords?: { lat: number; lng: number }) => {
    ...
    let targetCity = city ? city.trim() : "Seoul";
    ...
    // ✨ 2. 한글 도시명을 API가 알아먹을 수 있도록 영어로 변환해주는 마법의 선!
    ...
    if (koreanToEnglish[targetCity]) {
      targetCity = koreanToEnglish[targetCity];
    }
    ...
    try {
      ...
      // UI 화면에 노출될 도시는 오빠가 보낸 원본 한글/영문 이름 그대로 유지 처리
      const displayName = city.trim();

      const computedData: WeatherData = {
        location: displayName === "Seoul" ? "서울" : displayName === "Busan" ? "부산" : displayName === "Jeju" ? "제주" : (currentJson.name || city),
        source: "OpenWeather",
        current: {
          temp: tempVal,
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {POPULAR_CITIES.map((tag) => {
              // 한글 라벨과 영문 이름 모두 체크하도록 지능형 하이라이트 유지
              const isActive = weather?.location?.toLowerCase().includes(tag.label.toLowerCase()) || weather?.location?.toLowerCase().includes(tag.name.toLowerCase()) || (tag.label === "제주" && weather?.location === "제주");
              return (
